...
from src_files.helper_functions.helper_functions import mAP, CutoutPIL, ModelEma, TwoCropTransform, \
    adjust_learning_rate, warmup_learning_rate, add_weight_decay, get_datasets_from_csv, update_wordvecs
...
def main():
    ...
    train_backbone_transform = TwoCropTransform(transforms.Compose([
    ...
    val_backbone_transform = transforms.Compose([
                        transforms.Resize((args.image_size, args.image_size)),
                        transforms.RandomHorizontalFlip(),
                        transforms.ToTensor()
                        ])
    ...
    train_backbone_dataset, val_backbone_dataset, train_backbone_cls_ids, test_backbone_cls_ids = \
        get_datasets_from_csv(args.data,
                              args.data,
                              train_backbone_transform, val_backbone_transform,
                              json_path)
    ...
    train_dataset, val_dataset, train_cls_ids, test_cls_ids = \
        get_datasets_from_csv(args.data,
                              ...
                              json_path)
    train_backbone_wordvecs = wordvec_array[..., train_backbone_cls_ids].float()
    test_backbone_wordvecs = wordvec_array[..., test_backbone_cls_ids].float()
    ...
    print('Backbone train_cls_ids {} test_cls_ids {} '.format(train_backbone_cls_ids.shape, test_backbone_cls_ids.shape))
    print('head train_cls_ids {} test_cls_ids {} '.format(train_cls_ids.shape, test_cls_ids.shape))
    ...
    experiemnt_dir = os.path.join(args.save_dir, f"experiment_zsl_{args.run}")
    ...
    train_multi_label_coco_backbone_zsl(model, train_backbone_loader, val_backbone_loader,  args, train_backbone_wordvecs, test_backbone_wordvecs)
    if args.freeze:
    ...
    train_multi_label_coco_zsl(model, train_loader, val_loader,  args, train_wordvecs, test_wordvecs)


def train_multi_label_coco_backbone_zsl(model, train_loader, val_loader,  args, train_wordvecs=None,
                          test_wordvecs=None):
    ...
    wandb.init(
        ...
        name=f"backbone_training_zsl_{args.run}", 
        config={
    ...
    for epoch in range(1, args.epochs_con + 1):
        ...
        try:
            torch.save(model.state_dict(), os.path.join(
                *[args.save_dir, f"experiment_zsl_{args.run}", "models", "model-backbone-{}-{}.ckpt".format(epoch + 1, idx + 1)]))
        except:
            pass
...
def train_multi_label_coco_zsl(model, train_loader, val_loader, args, train_wordvecs=None,
                        test_wordvecs=None):
    if torch.cuda.is_available():
        model = model.cuda()
    ...
    wandb.init(
        project="test-project", 
        entity="pwr-multisupcontr",
        name=f"ML_Decode_training_zsl_{args.run}", 
        config={
    ...
    for epoch in range(args.epochs):
        ...
        try:
            torch.save(model.state_dict(), os.path.join(
                *[args.save_dir, f"experiment_zsl_{args.run}", "models", "model-{}-{}.ckpt".format(epoch + 1, idx + 1)]))
        except:
            pass
        ...
        if mAP_score > highest_mAP:
            ...
            try:
                torch.save(model.state_dict(), os.path.join(
                    *[args.save_dir, f"experiment_zsl_{args.run}", "models", "model-highest.ckpt"]))
            except:
                pass
